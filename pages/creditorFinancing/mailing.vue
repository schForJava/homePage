      ...
      <el-tabs v-model="activeName" @tab-click="handleClick">
        <el-tab-pane label="邮件报送" name="first">
                ...
                <el-col :span="11">
                  <el-form-item label="发行方式：" prop="jobType">
                    <el-radio-group
                      ...
                      @change="radioChange"
                    >
                      <el-radio class="radio" label="1" v-if="mailType"
                        >定时发送</el-radio
                      >
          ...
          >
            ...
            <div class="dialog-body" v-if="mailType">
              <span class="demonstration">时间段 :</span>
              <el-input-number
                v-model="dialogNumInput"
                @change="handleChange"
                :min="0"
                :max="10"
                label="描述文字"
            ...
            <el-row class="topCentent">
              ...
              >
                ...
                <el-form-item v-else label=" 收件人：">
                  <span v-html="checkForm.mailTo"></span>
                </el-form-item>
                <el-form-item v-if="jobType" prop="mailCc" key="2">
                  <div slot="label" style="margin-left: 10px">抄送：</div>
                  <el-input v-model="checkForm.mailCc"></el-input>
                </el-form-item>
                <el-form-item v-else label=" 抄送：">
                  <span v-html="checkForm.mailCc"></span>
                </el-form-item>
                <el-form-item
                  v-if="jobType"
                  label=" 标题："
                ...
                <el-form-item v-else label=" 标题：">
                  <span v-html="checkForm.subject"></span>
                </el-form-item>
                <el-form-item
                  v-if="jobType"
                  label=" 正文："
                ...
                <el-form-item v-else label=" 正文：">
                  <span v-html="checkForm.context"></span>
                </el-form-item>
                <el-form-item v-if="jobType">
                  <div slot="label" style="margin-left: 10px">签名落款：</div>
                  <el-input
                    type="textarea"
                    :rows="8"
                ...
                <el-form-item v-else label=" 签名落款：">
                  <span v-html="checkForm.mainSign"></span>
                </el-form-item>
                <el-form-item v-if="mailType" class="workReport">
                  <p class="work-title">债权融资计划挂牌业务工作汇报</p>
                  <p class="work-title">
                    {{ nowTimeYear }} (截止当日 {{ nowTimeHour }}:00 时数据)
                  </p>
                  <el-table
                    :data="workReport"
                    border
                  ...
                  >
                    ...
                    ></el-table-column>
                    <el-table-column
                      prop="listingNumberByWeak"
                      align="center"
                      label="发行金额（亿元）"
                    ></el-table-column>
                  </el-table>
                  <div class="comment">
                    备注：此数据为截止至本日 {{ nowTimeHour }}:00
                    时的挂牌数据，后续可能还有 {{ nowTimeHour }}:00
                    时未申购完成的数据。
                  </div>
                </el-form-item>
                <el-table
                ...
                >
                  ...
                  >
                    <template slot-scope="scope">
                      <span @click="downLoadPdf(scope.row)">
                        <a>{{ scope.row.fullName }}</a>
                      </span>
                    </template>
                  </el-table-column>
                </el-table>
        ...
        <el-tab-pane label="模板修改日志" name="changeLogs">
          ...
          <div class="tables">
            <el-table :data="changeLogData" border style="width: 100%">
              ...
              >
                <template slot-scope="scope">
                  <span v-if="scope.row.logType == 'a01'">邮件模板</span>
                </template>
              </el-table-column>
              <el-table-column
                prop="logOperator"
...
export default {
  ...
  data() {
    ...
    return {
      ...
      changeLogData: [],
      changeLogsValue: "",
      changeLogsOption: [{ value: "a01", label: "邮件模板" }],
      changeLog: {
        logItem1: "",
        logOperator: "",
        logTime: [],
  ...
  methods: {
    ...
    mailConfig(row) {
      ...
      getEmailConfig(params).then((res) => {
        let data = res.data;
        // console.log("==========================>" + data);
        this.bookConfig = {
          id: row.id,
    ...
    checkTime(time) {
      if (this.timeValue == "" || this.timeValue == null) {
        ...
        getMailShowMail(params).then((res) => {
          ...
          this.$nextTick(() => {
            ...
            if (this.checkRow.mailType == 2) {
              ...
              const list = res.data.listingBusinessReport;
              this.nowTimeYear = res.data.listingBusinessReport.nowTimeStr;
              this.nowTimeHour = res.data.listingBusinessReport.nowTimeHour;
              this.workReport = [
                {
                  name: "本周",
                  issueSclByWeak: list.listingNumberByWeak,
                  listingNumberByWeak: list.issueSclByWeak,
                },
                {
                  name: list.nowTimeYear + "年",
                  issueSclByWeak: list.listingNumberByYear,
                  listingNumberByWeak: list.issueSclByYear,
                },
              ];
            }
          });
    ...
    mailOneClickSend(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          getMailOneClickSend(this.checkForm).then((res) => {
    ...
    getMailData() {
      ...
      getMailPaperList(params).then((res) => {
        console.log(res);
