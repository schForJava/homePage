<template>
  <div class="edit12">
    <el-row class="count">
      <el-col :span="24">
        <div class="grid-content bg-purple-dark">
          <span class="count1">募集资金用途变更公告情况汇总表（日终）</span>
        </div>
      </el-col>
    </el-row>
    <div class="container">
      <!-- 标题 -->
      <el-row class="titleContent">
        <el-col :span="24">
          <i class="el-icon-back" @click="back" style="corsor: pointer"></i>
          <div
            class="grid-content bg-purple-dark"
            @click="back"
            style="corsor: pointer"
          >
            返回
          </div>
        </el-col>
      </el-row>
      <!--输入框1 -->
      <el-row class="topCentent">
        <el-col :span="6">
          <label class="search-label">债项全称：</label>
          <el-input
            placeholder="请输入"
            v-model="input.bondName"
            clearable
          ></el-input>
        </el-col>
        <el-col :span="6">
          <label class="search-label">债项类型：</label>
          <el-select v-model="valueOne" placeholder="请选择" clearable >
            <el-option
              v-for="(item,index) in optionOne"
              :key="index"
              :label="item.bondName"
              :value="item.bondAbbreviation"
            ></el-option>
          </el-select>
        </el-col>
        <el-col :span="6">
          <label class="search-label">发行人全称：</label>
          <el-input
            placeholder="请输入"
            v-model="input.orgName"
            clearable
          ></el-input>
        </el-col>
        <el-col :span="6">
          <label class="search-label">挂网主承销商：</label>
          <el-input
            placeholder="请输入"
            v-model="input.createUser"
            clearable
          ></el-input>
        </el-col>
      </el-row>
      <!--输入框4 -->
      <!--输入框1 -->
      <el-row class="topCentent" style="margin-top: 30px">
        <el-col :span="6">
          <label class="search-label">披露时间：</label>
          <el-date-picker
            class="dateinterval"
            value-format="yyyy-MM-dd"
            v-model="input.timingTime"
            type="daterange"
            range-separator="-"
            start-placeholder="开始日期"
            end-placeholder="结束日期"
            :default-value="defaultDate"
          ></el-date-picker>
        </el-col>
        <el-col :span="6">
          <label class="search-label">审核时间：</label>
          <el-date-picker
            class="dateinterval"
            value-format="yyyy-MM-dd"
            v-model="input.submitTime"
            type="daterange"
            range-separator="-"
            start-placeholder="开始日期"
            end-placeholder="结束日期"
            :default-value="defaultDate"
          ></el-date-picker>
        </el-col>
        <el-col :span="6">
          <label class="search-label">缴款日：</label>
          <el-date-picker
            class="dateinterval"
            value-format="yyyy-MM-dd"
            v-model="input.paymentDate"
            type="daterange"
            range-separator="-"
            start-placeholder="开始日期"
            end-placeholder="结束日期"
            :default-value="defaultDate"
          ></el-date-picker>
        </el-col>
      </el-row>
      <!-- 下半部分 -->
      <el-row class="bottomContent">
        <div class="btns">
          <el-button type="primary" @click="search">查询</el-button>
          <el-button type="primary" @click="dialogVisible = true"
            >导出</el-button
          >
          <el-button plain @click="reset">重置</el-button>
        </div>
      </el-row>
      <!-- <el-dialog :visible.sync="menuRoleVisible" append-to-body title="导入数据">
        <menu-role
          v-if="menuRoleVisible"
          ref="menuRole"
          @renewal="updateModal"
          @cencel="cencelModal"
        ></menu-role>
      </el-dialog>-->
      <!--tab部分 -->
      <div class="tables">
        <el-table
          v-loading="loading"
          element-loading-text="正在导出"
          :data="tableData"
          border
          style="width: 100%"
        >
          <el-table-column prop="id" label="序号" align="center" width="70px">
            <!-- <template slot-scope="scope">{{scope.$index+1}}</template> -->
          </el-table-column>
          <el-table-column
            prop="timingTime"
            width="200px"
            align="center"
            label="披露时间"
          ></el-table-column>
          <el-table-column
            prop="bondAbbreviation"
            width="200px"
            :show-overflow-tooltip="true"
            label="债项简称"
            header-align="center"
          ></el-table-column>
          <el-table-column
            prop="bondClassId"
            width="200px"
            :show-overflow-tooltip="true"
            label="债项类型"
            header-align="center"
          ></el-table-column>
          <el-table-column
            prop="bondName"
            width="300px"
            header-align="center"
            label="债项全称"
            :show-overflow-tooltip="true"
          ></el-table-column>
          <el-table-column
            prop="orgName"
            width="300px"
            header-align="center"
            label="发行人全称"
            :show-overflow-tooltip="true"
          ></el-table-column>
          <el-table-column
            prop="paymentDate"
            width="120px"
            align="center"
            label="缴款日"
          ></el-table-column>
          <el-table-column
            prop="maturityDate"
            width="120px"
            align="center"
            label="到期日"
          ></el-table-column>
          <el-table-column
            prop="issueFaceValue"
            width="150px"
            align="center"
            label="发行金额（万元）"
          ></el-table-column>
          <el-table-column
            prop="createUser"
            width="200px"
            label="挂网主承销商"
            :show-overflow-tooltip="true"
            align="center"
          ></el-table-column>
          <el-table-column
            prop="noticeTitle"
            width="300px"
            label="公告标题"
            header-align="center"
            :show-overflow-tooltip="true"
          >
            <template slot-scope="scope">
              <div slot="reference" class="name-wrapper noticeTitle">
                <a @click="handleOrder(scope.row)">
                  {{ scope.row.noticeTitle }}
                </a>
              </div>
            </template>
          </el-table-column>
          <el-table-column
            prop="submitTime"
            width="150px"
            align="center"
            label="审核时间"
          ></el-table-column>
        </el-table>
        <el-pagination
          @size-change="handleSizeChange"
          @current-change="handleCurrentChange"
          :current-page="pageNum"
          :page-sizes="pageSizes"
          :page-size="PageSize"
          layout="total, sizes, prev, pager, next, jumper"
          :total="totalCount"
        ></el-pagination>
      </div>
    </div>
    <!-- 导出弹窗 -->
    <el-dialog title="导出" :visible.sync="dialogVisible" width="30%">
      <div class="dialog-body">
        <span class="demonstration">*选择日期 :</span>
        <el-date-picker
          v-model="timeValue"
          type="date"
          placeholder="选择日期"
          value-format="yyyy-MM-dd"
        ></el-date-picker>
      </div>
      <span slot="footer" class="dialog-footer">
        <el-button @click="dialogVisible = false">取 消</el-button>
        <el-button type="primary" @click="derive">确 定</el-button>
      </span>
    </el-dialog>
    <!-- 披露附件 -->
    <el-dialog title="披露附件" :visible.sync="accessory" width="30%">
      <ul>
        <li
          class="pdfList"
          v-for="(item, index) of pdfList"
          :key="index"
          @click="
            downLoadPdf(item.GROUP_NAME, item.REMOTE_FILE_NAME, item.FILE_NAME)
          "
        >
          {{ item.FILE_NAME }}
        </li>
      </ul>
      <span slot="footer" class="dialog-footer">
        <el-button @click="accessory = false">关闭</el-button>
      </span>
    </el-dialog>
  </div>
</template>

<script>
  import {
    getInfo5,
    getInfoPdfList,
    getInfo5Select,
    getInfo5IfDownload, getSysDate,
  } from "../../library/api/api.public";
import { downloadUrl } from "../../library/api/axiosExport";

// import $http
import TableOne from "../home/components/TableOne";
import messageBox from "../home/components/messageBox.vue";
import MenuRole from "../home/components/MenuRole.vue";
export default {
  components: {
    TableOne,
    messageBox,
    MenuRole,
  },
  data() {
    return {
      loading: false,
      menuRoleVisible: false,
      dialogVisible: false,
      accessory: false,
      text: "确定",
      // from数据
      input: {
        timingTime: [], //披露时间
        bondAbbreviation: "", //债项简称
        bondClassId: "", //债项类型
        bondName: "", //债项全称
        orgName: "", //发行人全称
        paymentDate: "", //缴款日
        maturityDate: "", //到期日
        issueFaceValue: "", //发行金额
        createUser: "", //挂网主承销商
        noticeTitle: "", //公告标题
        submitTime: [], //审核时间
      },
      optionOne: "",
      valueOne: "",
      tableData: [],
      pageNum: 1,
      // 总条数，根据接口获取数据长度(注意：这里不能为空)
      totalCount: 0,
      // 个数选择器（可修改）
      pageSizes: [5, 10, 15, 20],
      // 默认每页显示的条数（可修改）
      PageSize: 5,
      timeValue: "",
      defaultDate: new Date(),
      pdfList: [],
      // 提示类型
      tipType: {
        warn: "warning",
        success: "success",
        error: "error",
      },
      // 提示信息
      tipInfo: {
        editTip: "当日无募集资金变更",
      },
    };
  },
  // getList?timingTimeStart=&timingTimeEnd=&createUser=恒丰银行股份有限公司&submitTimeStart=2019-06-11&submitTimeEnd=2019-06-11&paymentDateStart&paymentDateEnd
  // 获取初始数据
  created() {
    this.convertTime();
    this.getInfoSelect();
    // this.getData();
  },
  // 定义方法
  methods: {
    /**
     * 列表渲染
     */

    getData() {
      let paymentDateStart = "";
      let paymentDateEnd = "";
      let submitTimeStart = "";
      let submitTimeEnd = "";
      let timingTimeStart = "";
      let timingTimeEnd = "";
      if (this.input.paymentDate == "" || this.input.paymentDate == null) {
        paymentDateStart = "";
        paymentDateEnd = "";
      } else {
        paymentDateStart = this.input.paymentDate[0];
        paymentDateEnd = this.input.paymentDate[1];
      }
      if (this.input.submitTime == "" || this.input.submitTime == null) {
        submitTimeStart = "";
        submitTimeEnd = "";
      } else {
        submitTimeStart = this.input.submitTime[0];
        submitTimeEnd = this.input.submitTime[1];
      }
      if (this.input.timingTime == "" || this.input.timingTime == null) {
        timingTimeStart = "";
        timingTimeEnd = "";
      } else {
        timingTimeStart = this.input.timingTime[0];
        timingTimeEnd = this.input.timingTime[1];
      }
      let params = {
        pageSize: this.PageSize,
        pageNum: this.pageNum,
        timingTimeStart: timingTimeStart, //披露时间开始
        timingTimeEnd: timingTimeEnd, //披露时间结束
        bondClassId: this.valueOne, //债项类型
        bondName: this.input.bondName, //债项全称
        orgName: this.input.orgName, //发行人全称
        paymentDateStart: paymentDateStart, //缴款日
        paymentDateEnd: paymentDateEnd, //缴款日
        createUser: this.input.createUser, //挂网主承销商
        submitTimeStart: submitTimeStart, //审核时间开始
        submitTimeEnd: submitTimeEnd, //审核时间结束
      };
      getInfo5(params).then((res) => {
        console.log(res);
        this.tableData = res.data.list;
        this.totalCount = res.data.total;
      });
    },
    /**加载下拉框 */
    getInfoSelect() {
      let params = {};
      getInfo5Select(params).then((res) => {
        console.log(res);
        this.optionOne = res.data;
      });
    },
    /**
     * 查询条件
     */
    search() {
      this.pageNum = 1;
      this.getData();
    },
    //表格分页
    handleSizeChange(val) {
      this.PageSize = val;
      // 注意：在改变每页显示的条数时，要将页码显示到第一页
      this.pageNum = 1;
      this.getData();
      console.log(`每页 ${val} 条`);
    },
    handleCurrentChange(val) {
      this.pageNum = val;
      console.log(`当前页: ${val}`);
      this.getData();
    },

    /**
     * 搜索条件重置
     */
    reset() {
      this.input = {
        timingTime: [], //披露时间
        bondAbbreviation: "", //债项简称
        bondClassId: "", //债项类型
        bondName: "", //债项全称
        orgName: "", //发行人全称
        paymentDate: "", //缴款日
        maturityDate: "", //到期日
        issueFaceValue: "", //发行金额
        createUser: "", //挂网主承销商
        noticeTitle: "", //公告标题
        submitTime: [], //审核时间
      };
      this.valueOne = "";
      this.defaultDate = new Date();
      this.convertTime();
    },
    /**
     * 获取pdf List
     * @param publishId
     */
    handleOrder(row) {
      console.log(row);
      this.accessory = true;
      let params = { publishId: row.noticeTitle };
      getInfoPdfList(params).then((res) => {
        console.log(res.data);
        this.pdfList = res.data;
      });
    },
    downLoadPdf(GROUP_NAME, REMOTE_FILE_NAME, FILE_NAME) {
      this.dialogVisible = false;
      let url =
        this.$api.IP +
        "/purposeOFFunDsraised/fastDownLoad?groupName=" +
        GROUP_NAME;
      url += "&remoteFileName=" + REMOTE_FILE_NAME;
      url += "&fileName=" + FILE_NAME;
      window.location.href = url;
    },

    /**
       * 打开提示框
       @params message 提示框信息
       @params type 提示类型
       */
    openTipBox(message, type) {
      this.$message({
        showClose: true,
        message,
        type,
      });
    },

    /**
     * 导出功能
     */
    derive() {
      let params = {
        chooseTime: this.timeValue,
      };
      getInfo5IfDownload(params).then((res) => {
        console.log(res.data);
        if (res.data == "1") {
          this.dialogVisible = false;
          this.openTipBox(this.tipInfo.editTip, this.tipType.warn);
        } else {
          this.dialogVisible = false;
          let url =
            this.$api.IP +
            "/purposeOFFunDsraised/downLoadHisPurposeOfFunDsraised?chooseTime=" +
            this.timeValue;
          let param = {

            selectTime: this.timeValue,

          }
          getSysDate(param).then((res) => {
            let code = res.data.code;
            if (code == 1) {
              downloadUrl(url);
            } else if (code == 0) {
              let hour = res.data.hour;
              let minute = res.data.minute;
              if (hour > 17) {
                downloadUrl(url);
              } else if(hour == 17 && minute>30){
                downloadUrl(url);
              }else {
                this.$confirm('日终未到规定时间，请勿使用导出功能，是否确认继续导出？', '确认信息', {
                  distinguishCancelAndClose: false,
                  confirmButtonText: '导出',
                  cancelButtonText: '取消'
                }).then(() => {
                  this.$message({
                    type: 'success',
                    message: '导出数据'
                  });
                  downloadUrl(url);
                }).catch(action => {
                  this.$message({
                    type: 'info',
                    message: action === 'cancel'
                            ? '导出已取消'
                            : '导出已取消'
                  })
                });

              }
            }

          })
        }
      });
    },

    // //转换日期格式
    convertTime() {
      this.timeValue = new Date();
      var date = new Date(this.timeValue);
      var y = date.getFullYear();
      var m = date.getMonth() + 1;
      m = m < 10 ? "0" + m : m;
      var d = date.getDate();
      d = d < 10 ? "0" + d : d;
      var h = date.getHours();
      h = h < 10 ? "0" + h : h;
      var minute = date.getMinutes();
      minute = minute < 10 ? "0" + minute : minute;
      var second = date.getSeconds();
      second = second < 10 ? "0" + second : second;
      this.timeValue = y + "-" + m + "-" + d;
      let endTime = y + "-" + m + "-" + d;
      // this.input.submitTime.push(endTime);
      // this.input.submitTime.push(endTime);
    },
    // 点击返回
    back() {
      // 返回上一个页面
      this.$router.back(-1);
    },
  },
};
</script>

<style lang="scss" scoped>
.edit12 {
  margin-top: -1px;
  padding: 20px;
  font-size: 12px;

  /deep/ .el-dialog {
    display: flex;
    flex-direction: column;
    margin: 0 !important;
    position: absolute;
    top: 50%;
    left: 50%;
    transform: translate(-50%, -50%);
    max-height: calc(100% - 30px);
    max-width: calc(100% - 30px);
    .el-dialog__body {
      padding: 30px 25px;
      display: flex;
      justify-content: center;
    }
  }
  .count {
    font-size: 14px;
    .count2,
    .count3 {
      margin-left: 10px;
    }
    .count1,
    .count2 {
      color: #a3a3a3;
    }
  }
  // 下拉框部分
  .container {
    margin-top: 20px;
    padding: 20px;
    background-color: #fff;
    .titleContent {
      font-size: 12px;
      div {
        display: inline-block;
      }
    }
    // 上半部分的内容
    .topCentent {
      display: flex;
      margin-top: 9px;
      /deep/ .el-button.el-button--mini {
        border-radius: 8px;
      }
      .el-col {
        display: flex;
        .search-label {
          width: 50%;
          line-height: 36px;
          text-align: right;
          color: #515a5e;
        }
        /deep/ .el-input__inner {
          padding-right: unset;
          width: 100%;
          height: 28px;
          font-size: 12px;
        }
        .dateinterval {
          /deep/ .el-range-separator {
            width: 30px;
          }
        }
      }
      .btns {
        margin-left: auto;
        display: flex;
        // justify-content: center;
        // margin-left: 10px;
        button {
          border-radius: 5px !important;
          justify-content: center;
          margin-left: 10px;
        }
      }
      .toLead {
        padding: 0;
      }
    }
    // 下半部分
    .bottomContent {
      margin-top: 30px;
      @extend .topCentent;
    }
  }
  // tab部分
  .tables {
    margin-top: 20px;
    .cell {
      color: #65686e !important;
    }
    .tables /deep/.el-table__header-wrapper {
      height: 60px;
    }
  }
  .noticeTitle {
    color: blue;
    cursor: pointer;
    a {
      color: blue !important;
      text-decoration: underline;
    }
  }
  .dialog-body {
    display: flex;
    width: 300px;
    span {
      width: 100px;
      line-height: 36px;
    }
  }
  .pdfList {
    color: blue;
    text-decoration: underline;
  }
  button {
    margin-left: 8%;
    border-radius: 5px !important;
  }
}
</style>
