<template>
  <div class="widget-content">
    <el-modal v-model="importDataModal" title="导入数据">
      ...
      <el-row class="btns">
        <a :href="exportIP" download="台账表导入模板" class="adown"
          >点击下载：台账表导入模板.xlsx</a
        >
...
export default {
  data() {
    return {
      ...
      text: "确定",
      isLoding: false,
      exportIP: this.$api.exportIP + "/queryweb/downExcel?Code=bond",
    };
  },
  methods: {
    accessoryFileInput() {
    ...
    submitExcel() {
      ...
      if (fileName.indexOf("台账") < 0 || fileName.indexOf("导入") < 0) {
        this.$message("提示“文件名称包含“台账导入”字样”");
        this.isLoding = false;
        this.text = "确定";
        return true;
      }
