          ...
          >
            <div class="dialog-body">
              <ul>
                <p>成功数据</p>
                <li
                  v-for="(item, index) in rightResult"
                  :key="index"
                  :title="item"
                >
                  {{ item }}
...
import {
  getBookkeepingMail,
  getBookkeepingMailConfig,
  getBookkeepingMailUpdateMail,
  getBookkeepingMailShowMail,
  getBookkeepingMailOneClickSend,
  getBookkeepingMailPaperList,
  getBookkeepingMailListX,
  getSelectAllLog,
  getBookkeepingMailList,
...
export default {
  ...
  data() {
    ...
    return {
      ...
      changeLogData: [],
      changeLogsValue: "",
      changeLogsOption: [{ value: "a03", label: "邮件模板" }],
      changeLog: {
        logItem1: "",
        logOperator: "",
        logTime: [],
  ...
  methods: {
    ...
    getData() {
      ...
      getBookkeepingMail(params).then((res) => {
        console.log(res);
    ...
    exporMiddle(type) {
      if (this.exporR == "" || this.exporR == null) {
        ...
        window.location.href =
          this.$api.IP +
          "/BookkeepingMail/dowMailList?endOrMiddle=" +
          "1" +
          "&&jobTime=" +
          this.exporR;
      }
    ...
    sendMiddle() {
      if (this.Middle == "" || this.Middle == null) {
        ...
        getInfoDiscSendMailList(params).then((res) => {
          ...
          this.rightResult = res.data.rightResult;
    ...
    mailConfig(row) {
      ...
      getBookkeepingMailConfig(params).then((res) => {
        let data = res.data;
        // console.log("==========================>" + data);
        this.bookConfig = {
          id: row.id,
    ...
    updateMail(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          getBookkeepingMailUpdateMail(params).then((res) => {
            console.log("==========================>" + res);
            if (res.data > 0) {
    ...
    checkTime(time) {
      if (this.timeValue == "" || this.timeValue == null) {
        ...
        getBookkeepingMailShowMail(params).then((res) => {
    ...
    mailOneClickSend(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          getBookkeepingMailOneClickSend(this.checkForm).then((res) => {
    ...
    getMailData() {
      ...
      getBookkeepingMailPaperList(params).then((res) => {
        console.log(res);
