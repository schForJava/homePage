  ...
  <div class="widget-content">
    <el-modal v-model="importDataModal" title="导入数据">
      ...
      <el-row class="btns">
        <a
          :href="exportIp"
          download="债权融资计划发行规模导入模板"
          class="adown"
          >点击下载：债权融资计划发行规模导入模板.xlsx</a
        >
...
export default {
  data() {
    return {
      ...
      text: "确定",
      isLoding: false,
      exportIp: this.$api.exportIP + "/queryweb/downExcel?Code=cbs",
    };
  },
  methods: {
    accessoryFileInput() {
    ...
    submitExcel() {
      ...
      if (fileName.indexOf("债权融资计划发行规模") < 0) {
        this.$message("提示“文件名称包含“债权融资计划发行规模”字样”");
        this.isLoding = false;
        this.text = "确定";
        return true;
      }
